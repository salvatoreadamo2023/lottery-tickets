...
public class TicketService {
	...
	/**
	 * Restituisce i ticket applicando i filtri opzionali status/from/to. Gestisce
	 * tutte le combinazioni, incluso il caso in cui venga passato solo uno dei due
	 * estremi dell'intervallo temporale (from senza to, o viceversa): in tal caso
	 * il filtro è comunque applicato come "da from in poi" o "fino a to", invece di
	 * essere ignorato.
	 */
	...
	private Ticket findTicketOrThrow(String ticketId) {
		return ticketRepository.findByTicketId(ticketId).orElseThrow(() -> new TicketNotFoundException(ticketId));
	}
	 ...
	 * getAllTickets: from e to possono essere passati singolarmente senza che il
	 * filtro venga ignorato.
	 */
