...
public class SeedImportService implements CommandLineRunner {
	...
	public SeedImportService(TicketRepository ticketRepository, TicketAuditRepository ticketAuditRepository,
...
//run automatizzato con check se già esistente con dati la tab ticket
	@Override
	...
	private void importSeedFile() throws Exception {
		...
		log.info("Seed import completato: {} righe totali, {} importate, {} scartate", rows.size(), imported,
				rows.size() - imported);
	...
	private List<SeedRow> readRows() throws Exception {
		...
		try (InputStream is = resource.getInputStream(); Workbook workbook = new XSSFWorkbook(is)) {
			...
			for (int i = 1; i <= sheet.getLastRowNum(); i++) {
				...
				SeedRow seedRow = new SeedRow(i + 1, getCellValue(row.getCell(0)), getCellValue(row.getCell(1)),
...
//check row con logica validazione severa
	List<String> validateRowCompleto(SeedRow row, Map<String, Integer> ticketIdOccurrences,
			Map<String, Integer> privateCodeOccurrences) {
